...
class GNN_Shortner_Updater
{
     ...
     */
    private $repo = 'BigDesigner/gnn-shortner';

    /**
     * Plugin slug (directory name/main file).
     */
    private $plugin_slug = 'gnn-shortner/gnnshortner.php';

    /**
     * Transient key for caching the GitHub API response.
     */
    private $transient_key = 'gnn_shortner_github_update_check';
    ...
    {
        if ('plugin_information' !== $action || 'gnn-shortner' !== $args->slug) {
            return $result;
        }
    ...
    public function after_install($response, $hook_extra, $result)
    {
        ...
        $plugin_dir = WP_PLUGIN_DIR . '/gnn-shortner';
    ...
    {
        if (isset($_GET['gnn_shortner_check_update']) && '1' === $_GET['gnn_shortner_check_update']) {
            if (!isset($_GET['_wpnonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), 'gnn_shortner_manual_update')) {
                wp_die(esc_html__('Security check failed.', 'gnn-shortner'));
            }
